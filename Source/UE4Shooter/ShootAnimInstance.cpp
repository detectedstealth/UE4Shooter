...
UShootAnimInstance::UShootAnimInstance() :
	Speed(0.f),
	bIsInAir(false),
...
void UShootAnimInstance::UpdateAnimationProperties(float DeltaTime)
{
	...
	if (ShooterCharacter)
	{
		...
		bReloading = ShooterCharacter->GetCombatState() == ECombatState::ECS_Equipping;
...
void UShootAnimInstance::TurnInPlace()
{
...
void UShootAnimInstance::Lean(float DeltaTime)
{
	if (ShooterCharacter == nullptr) return;
